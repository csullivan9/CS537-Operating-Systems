...
void parse(char *argv[]){
	...
	char   **buff = calloc(BUFFER_SIZE, sizeof(char*)); //buffer for target line with all command lines
	...
	while(fgets(line, BUFFER_SIZE, fp)){
		...
		if(targetLine){	
			...
			if((new_line = strchr(line, '\r')) != NULL) {
				*new_line = '\0';
			}
		...
		//add all command lines to buffer with the target
			...
			if(buffEmpty) {
				//for(int i=0; i<num_cmds; i++){ //***free elements of buff
				//	free(buff[i]);
				//}
				//free(buff);
				num_cmds = 0;
				if(numTargets == 0) free(buff);
				buff = calloc(BUFFER_SIZE, sizeof(char*)); //***changed to calloc
				if(buff == NULL) {
			...
			//if((new_line = strchr(line, '\n')) != NULL || (new_line = strchr(line, '\r')) != NULL)
			//	*new_line = '\0';
			int u = sizeof(char*) * (strlen(line) + 1);
			buff[num_cmds] = malloc(u);
			bzero(buff[num_cmds], u);	//***
			strncpy(buff[num_cmds++], line, u-1); //add line to buffer
			buffEmpty = false;
		}
	} //end while loop
	
	//free allocated memory that is not needed anymore
	...
	for(int i = 0; i < it; i++) {
		...
		for(int j = 0; j < targetList[i]->num_deps; j++) {
			...
			if(match == 0) { //***added bzero, strncpy, (delete <=) condition in if statement*
				
				int s = strlen(targetList[i]->dependencies[j])+1;
				to_add = malloc(s);
				bzero(to_add, s);
				strncpy(to_add, targetList[i]->dependencies[j], s-1);
				
				char* delete;
				if((delete = strchr(to_add, '\r')) != NULL && delete <= (to_add + s)) {
					*delete = '\0';
				}
...
char* replaceFlags(char* ln, char* flags[], int num) {
	...
	char* args_pointer;
    char* flag;	
	char* args_replace = calloc(BUFFER_SIZE, sizeof(char)); //***changed to calloc
	//char* flag_pointer;
	char* flag_flag;
	char* flag_copy;
	char* copy; 
	...
	while(pointer != NULL) {
		...
		free(toFree); //***added to free copy
		toFree = copy;
...
char* substitute(char *str, char* to_replace, char *sub) {
	...
	free(str); //***added to fix memory leak occurring every time function is called
	return buffer;
}
